...
void ug()
{
	...
	do
	{
		...
		{
			std::cerr << "Ошибка: введённо негативное число или 0\n";
			exit = false;
		...
		{
			std::cerr << "Ошибка: значение месяца больше 12\n";
			exit = false;
		}
		...
		if (frst_month == 2 && LeapYearCheck(frst_year) == 1)	//valid day check
		{
			if (frst_day > 29)
			{
				std::cerr << "Ошибка: значение дня больше доступного в месяце\n";
				exit = false;
			}
		}
...
void ArrayNumTypeCount(int arr[], int size)	//третье задание
{
	...
	std::cout << "Положительных чисел: " << pos_count << "\nОтрицательных чисел: " << neg_count << "\nНулей: " << zero_count << "\n";
...
void FillSqrMtrx(int**& arr, int side, std::string overload_type)
{
...
int CommonDevis(int num1, int num2, int div)	//пятое задание
{
	if (div == 0)
	{
...
int BullsAndCows(std::string guess_num, int tries)	//шестое задание
{
	...
	do
	{
		...
		for (int i = 0; i < 4; i++)
		{
			if (int(u_inp[i]) < '0' || int(u_inp[i]) > '9' || u_inp.length() != 4)
			{
